...
/* ============================================================
   ZDR - DRAFT HEADER (RASCUNHO POR SETOR/MES)
   ============================================================ */
CREATE TABLE dbo.ZDR (
    ZDR_ID BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_ZDR PRIMARY KEY,
    ZDR_SETOR_ID INT NOT NULL,
    ZDR_COMPETENCIA DATE NOT NULL, -- YYYY-MM-01
    ZDR_STATUS NVARCHAR(20) NOT NULL CONSTRAINT DF_ZDR_STATUS DEFAULT ('RASCUNHO'), -- RASCUNHO|EM_REVISAO|APROVADO|REJEITADO
    ZDR_CRIADO_POR INT NOT NULL,
    ZDR_ENVIADO_POR INT NULL,
    ZDR_APROVADO_POR INT NULL,
    ZDR_REJEITADO_POR INT NULL,
    ZDR_MOTIVO_REJEICAO NVARCHAR(500) NULL,
    ZDR_CRIADO_EM DATETIME2(0) NOT NULL CONSTRAINT DF_ZDR_CRIADO DEFAULT (SYSUTCDATETIME()),
    ZDR_ATUALIZADO_EM DATETIME2(0) NOT NULL CONSTRAINT DF_ZDR_ATUALIZADO DEFAULT (SYSUTCDATETIME()),
    ZDR_ENVIADO_EM DATETIME2(0) NULL,
    ZDR_APROVADO_EM DATETIME2(0) NULL,
    ZDR_REJEITADO_EM DATETIME2(0) NULL,
    ZDR_LOCK_VERSION INT NOT NULL CONSTRAINT DF_ZDR_LOCK DEFAULT (1),
    CONSTRAINT FK_ZDR_ZSE FOREIGN KEY (ZDR_SETOR_ID) REFERENCES dbo.ZSE(ZSE_ID),
    CONSTRAINT FK_ZDR_CRIADOR FOREIGN KEY (ZDR_CRIADO_POR) REFERENCES dbo.ZFU(ZFU_ID),
    CONSTRAINT FK_ZDR_ENVIADOR FOREIGN KEY (ZDR_ENVIADO_POR) REFERENCES dbo.ZFU(ZFU_ID),
...
CREATE UNIQUE INDEX UX_ZDR_SETOR_COMPETENCIA ON dbo.ZDR (ZDR_SETOR_ID, ZDR_COMPETENCIA);
GO

CREATE INDEX IX_ZDR_STATUS ON dbo.ZDR (ZDR_STATUS, ZDR_COMPETENCIA);
GO

/* ============================================================
   ZDRI - DRAFT ITEMS (INDICADORES)
   ============================================================ */
CREATE TABLE dbo.ZDRI (
    ZDRI_ID BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_ZDRI PRIMARY KEY,
    ZDRI_DRAFT_ID BIGINT NOT NULL,
    ZDRI_INDICADOR_ID INT NOT NULL,
    ZDRI_VALOR_NUM DECIMAL(18,6) NULL,
    ZDRI_VALOR_TEXTO NVARCHAR(500) NULL,
    ZDRI_VALOR_DATA DATE NULL,
    ZDRI_PREENCHIDO_POR INT NULL,
    ZDRI_PREENCHIDO_EM DATETIME2(0) NULL,
    ZDRI_ATUALIZADO_EM DATETIME2(0) NULL,
    CONSTRAINT FK_ZDRI_ZDR FOREIGN KEY (ZDRI_DRAFT_ID) REFERENCES dbo.ZDR(ZDR_ID),
    CONSTRAINT FK_ZDRI_ZIN FOREIGN KEY (ZDRI_INDICADOR_ID) REFERENCES dbo.ZIN(ZIN_ID),
    CONSTRAINT FK_ZDRI_ZFU FOREIGN KEY (ZDRI_PREENCHIDO_POR) REFERENCES dbo.ZFU(ZFU_ID)
);
GO

CREATE UNIQUE INDEX UX_ZDRI_DRAFT_IND ON dbo.ZDRI (ZDRI_DRAFT_ID, ZDRI_INDICADOR_ID);
GO

CREATE INDEX IX_ZDRI_DRAFT ON dbo.ZDRI (ZDRI_DRAFT_ID) INCLUDE (ZDRI_INDICADOR_ID);
GO
