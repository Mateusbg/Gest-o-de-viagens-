...
def _get_or_create_setor(cur, setor_id, setor_nome):
    ...
    cur.execute(
        "INSERT INTO ZSE (ZSE_NOME, ZSE_ATIVO) OUTPUT INSERTED.ZSE_ID VALUES (?, 1)",
        (setor_nome,)
    )
    row = cur.fetchone()
    if not row or row[0] is None:
        raise RuntimeError("Falha ao inserir setor")
    return int(row[0])
...
def _get_or_create_funcionario(cur, funcionario_id, funcionario_email, funcionario_nome, setor_id=None):
    ...
    if funcionario_email:
        cur.execute("SELECT TOP 1 ZFU_ID FROM ZFU WHERE LOWER(ZFU_EMAIL) = LOWER(?)", (funcionario_email,))
        row = cur.fetchone()
        if row and row[0] is not None:
            return int(row[0])
    ...
    # cria um usuário básico (sem senha) apenas como "funcionário registrador"
    cur.execute(
        """
        INSERT INTO ZFU (ZFU_NOME, ZFU_EMAIL, ZFU_SETOR_ID, ZFU_NIVEL, ZFU_ATIVO, ZFU_CRIADO_EM)
        OUTPUT INSERTED.ZFU_ID
        VALUES (?, ?, ?, 1, 1, SYSUTCDATETIME())
        """,
        (funcionario_nome or funcionario_email, funcionario_email, setor_id)
    )
    row = cur.fetchone()
    if not row or row[0] is None:
        raise RuntimeError("Falha ao inserir funcionário")
    return int(row[0])
...
def _get_or_create_indicador(cur, indicador_id, setor_id, codigo, nome, tipo=None, unidade=None, meta=None):
    ...
    cur.execute(
        ...
        """,
        (int(setor_id), codigo, nome or f"Indicador {codigo}", tipo, unidade, meta)
    )
    row = cur.fetchone()
    if not row or row[0] is None:
        raise RuntimeError("Falha ao inserir indicador")
    return int(row[0])
